...
namespace Novactive\EzEnhancedImageAssetBundle\Imagine;
...
class AliasGenerator implements VariationHandler
{
    ...
    public function getVariation(Field $field, VersionInfo $versionInfo, $variationName, array $parameters = [])
    {
        ...
        if (IORepositoryResolver::VARIATION_ORIGINAL !== $variationName && !$this->ioResolver->isStored($originalPath, $variationName)) {
            try {
            ...
            $this->ioResolver->store(
                $this->applyFilter($originalBinary, $variationName, $parameters),
                $originalPath,
                $variationName
            );
        } else {
     ...
     */
    private function applyFilter(BinaryInterface $image, $variationName, array $config = [])
    {
        ...
        if (isset($filterConfig['reference']) && IORepositoryResolver::VARIATION_ORIGINAL !== $filterConfig['reference']) {
